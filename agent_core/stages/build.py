...
class Build(Stage):
    ...
    def run(self, ctx):
        print(f"[{self.name}] starting build process 👷")
        fixed_files = ctx.get("fixed_files", [])
        if not fixed_files:
            print(f"[{self.name}] No fixed files found to build.")
        ...
        for file_path_str in fixed_files:
            ...
            print(f"[{self.name}] Formatting {file_path} with black...")
            ...
            if not format_success:
                if "Command not found" in format_stderr or "Error running command" in format_stderr:
                    print(f"[{self.name}] Error running black on {file_path}: {format_stderr}")
                ...
                print(f"[{self.name}] {file_path} formatted successfully by black.")
                file_detail["formatter_status"] = "success"
                file_detail["formatter_output"] = format_stdout + format_stderr

            # 2. Linting with flake8
            print(f"[{self.name}] Linting {file_path} with flake8...")
        ...
        if build_results["status"] == "failure":
            print(f"[{self.name}] Build failed. Details: {build_results['details']}")
            # Optionally stop the pipeline
            # raise RuntimeError(f"[{self.name}] Build failed.")
        else:
            print(f"[{self.name}] Build completed. Status: {build_results['status']}")
